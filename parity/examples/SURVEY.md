...
| make-variables | 1 | 0 | `testapp`: $(COMPILATION_MODE) is not a modeled genrule Make variable (razel models SRCS/OUTS/locatio |
...
| rust-examples/04-ffi | 2 | 1 | `ffi`: error: Module has no symbol `rust_shared_library` |
| rust-examples/05-deps-cargo | 1 | 0 | ``: error: unsupported load path `@crates//:defs.bzl` (only //pkg:f.bzl, :f.bzl, or a vendored |
| rust-examples/06-deps-direct | 2 | 1 | `rest_tokio`: error: Module has no symbol `rust_doc` |
...
| rust-examples/08-grpc-client-server | 6 | 1 | `build/prost_toolchain`: error: Module has no symbol `rust_library_group` |
