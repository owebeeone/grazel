...
| D14 | `view-delta-roundtrip` (GR5) deferred: the View service isn't on the wire (razel-side, rides RR's View work with sched_hook counts). The smoke client proves hello+run+events meanwhile | GR5a | razel View service |
| D15 | taut's TYPESCRIPT backend lacks a runtime (types-only api.ts; client.ts imports a nonexistent `trial/ts` path) — gryth's REAL TS client blocked on taut-side work (GIANNI's repo, not an RR seam). The js backend (vendored cbor.js) carries GR5a | GR5a | taut TS runtime |
