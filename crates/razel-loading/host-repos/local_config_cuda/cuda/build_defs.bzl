# razel host materialization of @local_config_cuda//cuda:build_defs.bzl — the generator's
# template (xla/third_party/gpus/cuda/build_defs.bzl.tpl) with its own NO-CUDA substitutions
# (cuda_configure.bzl:542 — False/[]/[]/0.0). CPU-only host; snapshot upgrade path = L6a.
...
def if_cuda_newer_than(wanted_ver, if_true, if_false = []):
    ...
    configured_version = "0.0".rsplit("_", 1)[-1]
    ...
    if False and (wanted_major, wanted_minor) <= (configured_major, configured_minor):
      return if_true
    return if_false
