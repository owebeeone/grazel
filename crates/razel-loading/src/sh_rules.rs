...
//!
//! STUB. Registered in `rules::ruleset_modules` under the `@rules_shell//` prefix; a
//! `load("@rules_shell//...", ...)` resolves here. The native rule logic + tests are
...
/// The synthetic `@rules_shell` module re-exporting razel's native rules. Currently
...
pub(crate) fn module() -> Result<FrozenModule, String> {
    ...
    Module::with_temp_heap(|module| {
        let ast = AstModule::parse("@rules_shell", String::new(), &Dialect::Extended)
            .map_err(|e| format!("{e}"))?;
        {
