...
//!       [--rundir R] -- PROG [ARGS…]
//!   razel-process-wrapper rustc …   (P3.9 — not yet implemented)
...
fn dispatch(args: &[String]) -> Result<i32, String> {
    ...
    match sub.as_str() {
        ...
        // "rustc" => … (P3.9: apply a flags file to a rustc invocation)
        other => Err(format!("unknown subcommand `{other}` (expected `build-script`)")),
    }
}
