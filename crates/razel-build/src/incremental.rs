    ...
    pub fn configure_targets(&mut self, targets: Vec<AnalyzedTarget>) -> Result<(), String> {
        ...
        for t in &targets {
            ...
            for (i, act) in t.actions.iter().enumerate() {
                ...
                // The bar's per-action label: the rule's own description (crate + version for rust,
                // the source file for C++, …) when set, else a LANGUAGE-AGNOSTIC fallback — the
                // primary output's basename. Either way it's `mnemonic + a discriminating bit`; the
                // engine never parses target labels per-language (that lives in each rule).
                let disc = if act.description.is_empty() {
                    act.outputs.first().map_or("", |o| o.rsplit('/').next().unwrap_or(o))
                } else {
                    act.description.as_str()
                };
                let desc = format!("{} {}", act.mnemonic, disc);
